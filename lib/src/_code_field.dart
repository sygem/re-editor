...
class _CodeFieldRender extends RenderBox implements MouseTrackerAnnotation {
  ...
  int get linesPerPage {
    ...
    return (pageSize / lineHeight).ceil();
  }
  ...
  CodeLineSelection? extendPositionTo({
    ...
    final CodeLinePosition? result = calculateTextPosition(Offset(min(size.width, max(0, localPosition.dx)),
    ...
    return oldSelection.copyWith(
        extentIndex: result.index, extentOffset: result.offset, extentAffinity: result.affinity);
  }
    ...
    if (offset == null) {
      if (_displayParagraphs.isNotEmpty) {
        ...
        if (position.index > last.index) {
          _verticalViewport
              .jumpTo(max(0, last.bottom - size.height + _preferredLineHeight * (position.index - first.index)));
        }
      }
      if (tryCount < 10) {
  ...
  CodeLinePosition? getDownPosition(CodeLinePosition position) {
    ...
    return CodeLinePosition.from(index: position.index + 1, position: downParagraph.getPosition(Offset(offset.dx, 0)));
  }
      ...
      if (span is MouseTrackerAnnotationTextSpan) {
        result.add(HitTestEntry(_MouseTrackerAnnotationTextSpan(
          ...
          rects: paragraph.getRangeRects(paragraph.getRangeForSpan(span)).map((rect) {
            return Rect.fromPoints(localToGlobal(rect.topLeft + paragraph.offset - paintOffset),
                localToGlobal(rect.bottomRight + paragraph.offset - paintOffset));
          }).toList(),
          span: span,
        )));
  ...
  void _updateDisplayRenderParagraphs() {
    ...
    final double totalHeight = _displayParagraphs.last.bottom +
        (_codes.length - (_displayParagraphs.last.index + 1)) * _preferredLineHeight +
        paddingBottom;
  ...
  void _updateSelectionExtentsVisibility(Offset effectiveOffset) {
    ...
    if (startOffset == null) {
      ...
      _selectionStartInViewport.value = visibleRegion.inflate(visibleRegionSlop).contains(startOffset);
    }
      ...
      for (final CodeLineRenderParagraph paragraph in paragraphs) {
        ...
        if (startIndex == endIndex) {
          ...
          if (offset == null) {
            ...
            rects = [Rect.fromLTWH(offset.dx, offset.dy, 0, paragraph.preferredLineHeight)];
          }
        } else {
...
class _CodeFieldSelectionPainter extends _CodeFieldSelectionsPainter {
  _CodeFieldSelectionPainter(Color color, CodeLineSelection selection) : super(color, [selection]);
...
class _CodeFieldCursorPainter extends _CodeFieldExtraPainter {
  ...
  void _drawCaret(Canvas canvas, Offset offset, Size size) {
    _paint.color = _color;
    canvas.drawRRect(
        RRect.fromRectXY(Rect.fromLTWH(offset.dx - _width / 2, offset.dy, _width, _height), _width / 2, _width / 2),
        _paint);
  }
}
