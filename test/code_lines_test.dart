...
void main() {
  group('CodeLines constructor ', () {
    test('`CodeLines()`', () {
      ...
      }
      {
        const CodeLines codeLines = CodeLines([
          CodeLineSegment(
              codeLines: [CodeLine('abc'), CodeLine('foo'), CodeLine('bar')])
        ]);
        expect(
            listEquals(codeLines.segments, const [
    ...
    test('`CodeLines.of()`', () {
      ...
      }
      {
        final CodeLines codeLines =
            CodeLines.of(List.generate(512, (index) => CodeLine('$index')));
  ...
  group('CodeLines getter ', () {
    test('`first` and `last`', () {
      ...
      {
        final CodeLines codeLines =
            CodeLines.of(List.generate(3000, (index) => CodeLine('$index')));
  ...
  group('CodeLines operator ', () {
    ...
    test('`[]=`', () {
      ...
      }
      {
        ...
        expect(() => codeLines[1] = const CodeLine('abc'),
            throwsA(isA<RangeError>()));
      }
      {
        final CodeLines codeLines = CodeLines.of(
    ...
    test('`==`', () {
      ...
      expect(CodeLines.empty() == CodeLines.of(const [CodeLine('abc')]), false);
      expect(
          CodeLines.of(const [CodeLine('abc')]) ==
              CodeLines.of(const [CodeLine('abc')]),
          true);
      expect(
          CodeLines.of(const [CodeLine('foo')]) ==
              CodeLines.of(const [CodeLine('abc')]),
          false);
      expect(
          const CodeLines([
                CodeLineSegment(codeLines: [CodeLine('abc')])
              ]) ==
              const CodeLines([
                CodeLineSegment(codeLines: [CodeLine('abc')])
              ]),
      ...
      expect(
          const CodeLines([
                CodeLineSegment(codeLines: [CodeLine('abc')]),
                CodeLineSegment(codeLines: [CodeLine('foo')]),
                CodeLineSegment(codeLines: [CodeLine('bar')])
              ]) ==
              const CodeLines([
                CodeLineSegment(codeLines: [CodeLine('abc')]),
                CodeLineSegment(codeLines: [CodeLine('foo')]),
      ...
      expect(
          const CodeLines([
                CodeLineSegment(codeLines: [CodeLine('abc')], dirty: true),
                CodeLineSegment(codeLines: [CodeLine('foo')]),
                CodeLineSegment(codeLines: [CodeLine('bar')])
              ]) ==
              const CodeLines([
  ...
  group('CodeLines method ', () {
    test('`add()`', () {
      ...
      {
        final CodeLines codeLines =
            CodeLines.of(List.generate(1024, (index) => CodeLine('$index')));
        codeLines.add(const CodeLine('abc'));
        expect(codeLines.length, 1025);
        expect(codeLines.last, const CodeLine('abc'));
        expect(codeLines.segments.length, 5);
        expect(codeLines.segments.last,
            const CodeLineSegment(codeLines: [CodeLine('abc')]));
      }
      {
        final CodeLines codeLines = CodeLines([
          CodeLineSegment(
              codeLines: List.of(const [CodeLine('abc')]), dirty: true)
        ]);
        codeLines.add(const CodeLine('foo'));
        codeLines.add(const CodeLine('bar'));
        expect(codeLines.length, 3);
    ...
    test('`addAll()`', () {
      {
        final CodeLines codeLines = CodeLines.empty();
        codeLines.addAll(const [CodeLine('abc')]);
        expect(codeLines.length, 1);
        expect(codeLines.first, const CodeLine('abc'));
        expect(codeLines.last, const CodeLine('abc'));
        codeLines.addAll(const [CodeLine('foo'), CodeLine('bar')]);
      }
      {
        final CodeLines codeLines = CodeLines.of(const [CodeLine('abc')]);
    ...
    test('`addFrom()`', () {
      {
        ...
        final CodeLines codeLines2 = CodeLines.of(const [CodeLine('abc')]);
        codeLines1.addFrom(codeLines2, 0);
        expect(
            listEquals(codeLines1.segments, const [
              CodeLineSegment(codeLines: [CodeLine('abc')], dirty: true)
            ]),
            true);
      }
      {
        ...
        expect(
            listEquals(codeLines2.segments, const [
              CodeLineSegment(
                codeLines: [CodeLine('123'), CodeLine('foo')],
              ),
            ]),
            true);
      ...
      {
        final CodeLines codeLines1 = CodeLines.of(const [CodeLine('abc')]);
        final CodeLines codeLines2 =
            CodeLines.of(List.generate(512, (index) => CodeLine('$index')));
        codeLines1.addFrom(codeLines2, 0);
        expect(
            listEquals(codeLines1.segments, [
              const CodeLineSegment(
                codeLines: [CodeLine('abc')],
              ),
              CodeLineSegment(
              ...
              CodeLineSegment(
                  codeLines:
                      List.generate(256, (index) => CodeLine('${index + 256}')),
                  dirty: true),
            ]),
            true);
      }
      {
        ...
        final CodeLines codeLines2 = CodeLines([
          const CodeLineSegment(
            codeLines: [CodeLine('foo')],
          ),
          const CodeLineSegment(
            codeLines: [CodeLine('bar')],
          ),
          CodeLineSegment(
            codeLines: List.generate(256, (index) => CodeLine('$index')),
          ),
        ...
        expect(
            listEquals(codeLines1.segments, [
              ...
              CodeLineSegment(
                  codeLines: List.generate(256, (index) => CodeLine('$index')),
                  dirty: true),
              const CodeLineSegment(codeLines: [CodeLine('abc')], dirty: true),
            ]),
            true);
      }
    });
    ...
    test('`asString`', () {
      ...
      }
      {
        ...
        expect(codeLines.asString(TextLineBreak.lf),
            'abc\n123\n456\n789\nfoo\n123\n456\n789\nbar\n123\n456\n789');
        expect(codeLines.asString(TextLineBreak.crlf),
            'abc\r\n123\r\n456\r\n789\r\nfoo\r\n123\r\n456\r\n789\r\nbar\r\n123\r\n456\r\n789');
        expect(codeLines.asString(TextLineBreak.cr),
            'abc\r123\r456\r789\rfoo\r123\r456\r789\rbar\r123\r456\r789');
    ...
    test('`equals()`', () {
      ...
      expect(CodeLines.empty().equals(CodeLines.of(const [CodeLine('abc')])),
          false);
      expect(CodeLines.of(const [CodeLine('abc')]).equals(CodeLines.empty()),
          false);
      expect(
          CodeLines.of(const [CodeLine('abc')])
              .equals(CodeLines.of(const [CodeLine('abc')])),
          true);
      expect(
          CodeLines.of(
                  const [CodeLine('abc'), CodeLine('foo'), CodeLine('bar')])
    ...
    test('`sublines()`', () {
      ...
      }
      {
        ...
        expect(codeLines.sublines(0) != codeLines, true);
        expect(
            codeLines.sublines(1),
            const CodeLines([
              CodeLineSegment(
                codeLines: [CodeLine('foo'), CodeLine('bar')],
              )
            ]));
        expect(
            codeLines.sublines(0, 1),
            const CodeLines([
              CodeLineSegment(
                codeLines: [
                  CodeLine('abc'),
      ...
      }
      {
        final CodeLines codeLines =
            CodeLines.of(List.generate(3000, (index) => CodeLine('$index')));
        expect(
            codeLines.sublines(512, 1024 + 512),
            CodeLines([
              CodeLineSegment(
                  codeLines:
                      List.generate(256, (index) => CodeLine('${index + 512}')),
                  dirty: true),
              CodeLineSegment(
                  codeLines: List.generate(
                      256, (index) => CodeLine('${index + 512 + 256 * 1}')),
                  dirty: true),
              CodeLineSegment(
                  codeLines: List.generate(
                      256, (index) => CodeLine('${index + 512 + 256 * 2}')),
                  dirty: true),
              CodeLineSegment(
                  codeLines: List.generate(
                      256, (index) => CodeLine('${index + 512 + 256 * 3}')),
                  dirty: true)
            ]));
        expect(
            codeLines.sublines(512, 2048 + 512),
            CodeLines([
              CodeLineSegment(
                  codeLines:
                      List.generate(256, (index) => CodeLine('${index + 512}')),
              ...
              CodeLineSegment(
                  codeLines: List.generate(
                      256, (index) => CodeLine('${index + 512 + 256 * 4}')),
                  dirty: true),
              CodeLineSegment(
                  codeLines: List.generate(
                      256, (index) => CodeLine('${index + 512 + 256 * 5}')),
                  dirty: true),
              CodeLineSegment(
                  codeLines: List.generate(
                      256, (index) => CodeLine('${index + 512 + 256 * 6}')),
                  dirty: true),
              CodeLineSegment(
                  codeLines: List.generate(
                      256, (index) => CodeLine('${index + 512 + 256 * 7}')),
                  dirty: true)
            ]));
      }
    });
